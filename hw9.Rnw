...
roots.frame <- data.frame(Root = format(roots, digits = 3),
                          ...
                          QP = ifelse(zapsmall(Im(roots)) == 0, NA,
                                      2 * pi / abs(Arg(roots))))
print(xtable(roots.frame[order(roots.frame$QP),], align = "lrrr"),
...
{\bf Check how to interpret ar coefficients. What does he mean by the actual estimated intercept vs. the estimate of the intercept? Why is this not Poisson?}
...
Of the 16 lower order terms, 14 were not detectably different from 0.
